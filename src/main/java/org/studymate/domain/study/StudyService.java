...
public class StudyService {
	...
	// 스터디 목록 확보하는 서비스
	@Transactional
	public List<Study> getStudyListByUser(Long userId) {
		var list = studyRepository.findByUserId(userId);
	...
	public void addNoticeToStudy(Long userId, String studyId, CreateNoticeRequest createNoticeDto) {
		...
		var notice = Notice.builder().description(createNoticeDto.getDescription()).study(study).tag(createNoticeDto.getTag()).writed(LocalDateTime.now()).build();
